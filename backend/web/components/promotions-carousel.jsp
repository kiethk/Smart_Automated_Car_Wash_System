...
<div class="carousel-container relative w-full group overflow-hidden rounded-2xl shadow-sm border border-slate-100">
    ...
    <div class="promo-slider flex overflow-x-auto scroll-smooth snap-x snap-mandatory no-scrollbar">
        <%
            List<Promotion> promos = (List<Promotion>) request.getAttribute("PROMOTIONS_LIST");
            if (promos != null) {
                for (int i = 0; i < promos.size(); i++) {
                    Promotion promo = promos.get(i);

                    // Đặt mặc định ảnh nếu không có ảnh
                    String bgImage = (promo.getImageUrl() != null && !promo.getImageUrl().trim().isEmpty())
                            ? promo.getImageUrl()
                            : "/assets/images/car-background.jpg";

                    // BỘ LỌC ĐƯỜNG DẪN ẢNH CLOUDINARY BẰNG JAVA THUẦN
                    String finalImgUrl = bgImage;
                    if (!bgImage.startsWith("http://") && !bgImage.startsWith("https://")) {
                        finalImgUrl = request.getContextPath() + bgImage;
                    }
        %>
        ...
        <div class="slide-item snap-start shrink-0 w-full bg-cover bg-center p-8 md:p-12 text-white relative min-h-[210px] md:min-h-[250px] flex flex-col justify-center transition-all duration-300"
            ...
            <div class="relative z-10 space-y-3 max-w-xl text-left">
                ...
                <div class="pt-2">
                    <a href="<%= request.getContextPath()%>/MainController?action=booking&code=<%= promo.getCode()%>" 
    ...
    <div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-20">
        <%
            if (promos != null) {
                for (int i = 0; i < promos.size(); i++) {
        %>
        <button data-index="<%= i%>" class="dot-indicator h-2 rounded-full bg-white/40 transition-all duration-300 focus:outline-none <%= (i == 0) ? "bg-white w-5" : "w-2"%>">
        </button>
        <%
                }
            }
