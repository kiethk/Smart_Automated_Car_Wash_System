                ...
                <div class="flex justify-between items-start mb-8">
                    <div>
                        ...
                        <%
                            ...
                            String currentTierName = (String) request.getAttribute("CURRENT_TIER_NAME");
                        %>
                        <h2 class="text-2xl font-bold tracking-tight"><%= user != null ? user.getFullName() : ""%></h2>
                    </div>
                    <span class="bg-amber-400 text-amber-950 text-[10px] font-extrabold px-3 py-1 rounded-full flex items-center gap-1 shadow-sm uppercase tracking-wider">
                        ★ <%= currentTierName != null ? currentTierName : ""%> Tier
                    </span>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 pt-4 border-t border-white/10">
                    <div>
                        ...
                        <p class="text-xl font-mono font-bold tracking-wide">
                            ...
                            %>
                            <%= customer != null ? df.format(customer.getTotalPoints()) : "0"%>
                        </p>
                    </div>
                        ...
                        <div>
                            ...
                            <p class="text-xl font-mono font-bold tracking-wide">
                                <%
                                    ...
                                    // Giá trị tiền Wallet của bạn lúc nãy định dạng tắt grouping (groupingUsed="false") nên sẽ in số nguyên
%>
                                <%= wallet != null ? String.valueOf((long) wallet.getBalance()) : "0"%> VND
                            </p>
                        </div>
                        <a href="<%= request.getContextPath()%>/MainController?action=topup" 
            ...
            <div class="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm">
                <div class="flex justify-between text-xs font-bold text-slate-600 mb-2">
                    ...
                    <span class="text-slate-500 font-medium">
                        <%
                            Boolean isMaxTier = (Boolean) request.getAttribute("IS_MAX_TIER");
                            if (isMaxTier != null && isMaxTier) {
                        %>
                        You have reached the highest <strong class="text-slate-800 font-bold">Platinum</strong> elite status!
                        <% } else {
                            Integer washesLeft = (Integer) request.getAttribute("WASHES_LEFT");
                            String nextTierName = (String) request.getAttribute("NEXT_TIER_NAME");
                        %>
                        <%= washesLeft != null ? washesLeft : 0%> more washes until <strong class="text-slate-800 font-bold"><%= nextTierName != null ? nextTierName : ""%></strong>
                        <% } %>
                    </span>
                </div>
                <%
                    Integer progressPercent = (Integer) request.getAttribute("PROGRESS_PERCENT");
                    int percent = (progressPercent != null) ? progressPercent : 0;
                %>
            ...
            <div class="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm flex flex-col justify-between min-h-[300px]">
                <div>
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <%
                            Boolean hasAppointment = (Boolean) request.getAttribute("HAS_APPOINTMENT");
                            Map<String, Object> appointment = null;
                            if (hasAppointment != null && hasAppointment) {
                                appointment = (Map<String, Object>) request.getAttribute("APPOINTMENT");
                        %>
                        <span class="bg-emerald-50 text-emerald-700 border border-emerald-200 text-[10px] font-bold px-2.5 py-1 rounded-full flex items-center gap-1 capitalize">
                            ...
                            <%= appointment != null ? appointment.get("status") : ""%>
                        </span>
                        <% } else { %>
                    ...
                    <div class="space-y-5">
                        <% if (hasAppointment != null && hasAppointment && appointment != null) {%>
                        <div class="flex items-start gap-3.5">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-semibold text-slate-800">
                                    <%= appointment.get("bookingDate")%> at <%= appointment.get("timeValue")%>
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start gap-3.5">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-semibold text-slate-800 font-mono">
                                    <%= appointment.get("plateNumber")%> (<%= appointment.get("brandDisplay")%> <%= appointment.get("modelDisplay")%>)
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start gap-3.5">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-semibold text-slate-800">
                                    <%
                                        Object bayNameObj = appointment.get("bayName");
                                        String bayName = (bayNameObj != null) ? bayNameObj.toString() : "Assigning Automatically...";
                                    %>
                                    <%= bayName%>
                                </p>
                            </div>
